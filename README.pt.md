...
**Importante:** Antes de testar os endpoints, uma chave de API precisa ser inserida no banco de dados. Veja as instruções detalhadas na seção **Fluxo de autenticação** abaixo.
...
O endpoint raiz redireciona para a documentação Swagger em `/docs`.
...
Retorno esperado da API FastAPI:
...
Antes de chamar endpoints protegidos, certifique-se de que a tabela `clients` contém pelo menos um cliente com uma API key válida, por exemplo:
...
Sem isso, as requisições retornarão `401`, porque a dependência de autenticação valida `clients.api_key` antes de criar o job.
